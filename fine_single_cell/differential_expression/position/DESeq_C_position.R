...
celltypes_bulk <- readRDS(file.path(data_dir,"fine_single_cell",
...
# Subset ss to include only the CONTROL samples -------------------------------
for (celltype in names(celltypes_bulk)){
  
  # get the celltype
  ss <- ss_list[[celltype]]
  
  # subset to include only CONTROL samples
  ss_c <- ss[ss$condition == "C",]
  
  # print to check
  print(ss_c)
  
  # add to the ss_list
  ss_list[[celltype]] <- ss_c
...
for (celltype in names(celltypes_bulk)){
  ...
  # c samples 
  c_samples <- ss_list[[celltype]]$sample
  
  # subset to include only anterior samples
  em_c <- em[, c("ID", c_samples)]
  
  # print to check
  print(em_c)
  
  # add to the em_list
  em_list[[celltype]] <- em_c
...
for (celltype in names(celltypes_bulk)) {
  ...
  tryCatch({
    # base path
    base_path <- file.path(data_dir,"fine_single_cell","differential_expression", "position", "control")
...
# SPECIAL LOOP for Immune.4 cluster --------------------------------------------

special.cases <- "Immune.4"
# in a loop
for (celltype in special.cases) {
  ...
  tryCatch({
    ...
    dds <- DESeqDataSetFromMatrix(countData = em[,ss$sample], 
                                  colData = ss,
                                  design = ~ sample_group) # removed mouse_id as was unnecesary here
