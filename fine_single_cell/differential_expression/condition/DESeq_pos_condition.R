...
# Subset ss to include only the posterior samples -------------------------------
for (celltype in names(celltypes_bulk)){
  
  # get the celltype
  ss <- ss_list[[celltype]]
  
  # subset to include only posterior samples
  ss_pos <- ss[ss$position == "pos",]
  
  # print to check
  print(ss_pos)
  
  # add to the ss_list
  ss_list[[celltype]] <- ss_pos
...
# subset list of em tables to include only posterior samples ----------------------
for (celltype in names(celltypes_bulk)){
  ...
  # pos samples 
  pos_samples <- ss_list[[celltype]]$sample
  
  # subset to include only posterior samples
  em_pos <- em[, c("ID", pos_samples)]
  
  # print to check
  print(em_pos)
  
  # add to the em_list
  em_list[[celltype]] <- em_pos
...
for (celltype in names(celltypes_bulk)) {
  ...
  tryCatch({
    # base path
    base_path <- file.path(data_dir,"fine_single_cell","differential_expression", "condition", "posterior")
    ...
    dds$sample_group <- relevel(dds$sample_group, ref = "pos_C")
