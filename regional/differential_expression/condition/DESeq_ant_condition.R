...
for (region in names(regions_bulk)){
  ...
  # ant samples 
  ant_samples <- ss_list[[region]]$sample
...
for (region in names(regions_bulk)) {
  ...
  tryCatch({
    # base path
    base_path <- file.path(data_dir,"regional","differential_expression", "condition", "anterior")
