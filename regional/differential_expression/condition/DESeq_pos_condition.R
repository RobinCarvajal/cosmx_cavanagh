...
for (region in names(regions_bulk)){
  ...
  # pos samples 
  pos_samples <- ss_list[[region]]$sample
...
for (region in names(regions_bulk)) {
  ...
  tryCatch({
    # base path
    base_path <- file.path(data_dir,"regional","differential_expression", "condition", "posterior")
