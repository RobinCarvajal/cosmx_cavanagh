...
The design formula `~ mouse_id + position*condition` in DESeq2 includes:

`mouse_id`: This accounts for any systematic differences due to mouse_id (oversampling). 
`condition`: This models the primary effect of the condition after controlling for position. 
`position:condition`: This interaction term allows the effect of condition to vary depending on position.
