...
---
# DESeq2 Immune - position+condition
...
base_dir <- "F:/cosmx_data/broad_single_cell/differential_expression/position+condition/Immune"
# load em 
counts <- read.table(file.path(base_dir, "em_Immune.tsv"), header = T, sep = "\t", row.names = 1)
# load ss
ss <- read.table(file.path(base_dir, "ss_Immune.tsv"), header = T, sep = "\t", row.names = 1)
```
...
dds <- DESeqDataSetFromMatrix(countData = counts, 
                              colData = ss,
                              design = ~ position + condition + position:condition)
...
dds$position <- relevel(dds$position, ref = "ant")
...
Mainly looking for any interaction between position and condition

```{r}
# results
res <- results(dds, contrast = list("positionpos.conditionA"))
...
There are NO genes that are uniquely changing in posterior slice samples due to Aldara treatment in IMMUNE cells.
